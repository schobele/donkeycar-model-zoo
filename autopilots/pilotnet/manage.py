...
from donkeycar.parts.conditional import ConditionalPart
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if cfg.HAVE_ODOM:
        if cfg.ENCODER_TYPE == "GPIO":
            ...
            enc = RotaryEncoder(mm_per_tick=0.306096,
                                pin=cfg.ODOM_PIN, debug=cfg.ODOM_DEBUG)
            V.add(enc, inputs=['throttle'], outputs=[
                  'enc/speed'], threaded=True)
    ...
    if camera_type == "stereo":
        ...
        if cfg.CAMERA_TYPE == "WEBCAM":
            ...
            camA = Webcam(image_w=cfg.IMAGE_W, image_h=cfg.IMAGE_H,
                          image_d=cfg.IMAGE_DEPTH, iCam=0)
            camB = Webcam(image_w=cfg.IMAGE_W, image_h=cfg.IMAGE_H,
                          image_d=cfg.IMAGE_DEPTH, iCam=1)

        elif cfg.CAMERA_TYPE == "CVCAM":
            from donkeycar.parts.cv import CvCam

            camA = CvCam(image_w=cfg.IMAGE_W, image_h=cfg.IMAGE_H,
                         image_d=cfg.IMAGE_DEPTH, iCam=0)
            camB = CvCam(image_w=cfg.IMAGE_W, image_h=cfg.IMAGE_H,
                         image_d=cfg.IMAGE_DEPTH, iCam=1)
        else:
        ...
        if cfg.DONKEY_GYM:
            from donkeycar.parts.dgym import DonkeyGymEnv
            # rbx
            cam = DonkeyGymEnv(cfg.DONKEY_SIM_PATH, host=cfg.SIM_HOST, env_name=cfg.DONKEY_GYM_ENV_NAME, conf=cfg.GYM_CONF, record_location=cfg.SIM_RECORD_LOCATION,
                               record_gyroaccel=cfg.SIM_RECORD_GYROACCEL, record_velocity=cfg.SIM_RECORD_VELOCITY, record_lidar=cfg.SIM_RECORD_LIDAR, delay=cfg.SIM_ARTIFICIAL_LATENCY)
            threaded = True
            ...
            cam = MockCamera(image_w=cfg.IMAGE_W,
                             image_h=cfg.IMAGE_H, image_d=cfg.IMAGE_DEPTH)
...
# This web controller will create a web server that is capable
        ...
        if cfg.CONTROLLER_TYPE == "pigpio_rc":
            ...
            if cfg.CONTROLLER_TYPE == "custom":  # custom controller created with `donkey createjs` command
                ...
                if cfg.USE_NETWORKED_JS:
                    ...
                    ctr.js = netwkJs
            V.add(ctr, inputs=['cam/image_array'], outputs=['user/angle',
        ...
        def run(self, mode, recording, recording_alert, behavior_state, model_file_changed, track_loc):
            ...
            if model_file_changed:
                led.set_rgb(self.cfg.MODEL_RELOADED_LED_R,
                            self.cfg.MODEL_RELOADED_LED_G, self.cfg.MODEL_RELOADED_LED_B)
                return 0.1
            else:
    ...
    if cfg.TRAIN_BEHAVIORS:
        ...
        V.add(bh, outputs=['behavior/state',
              'behavior/label', "behavior/one_hot_state_array"])
        try:
            ctr.set_button_down_trigger('L1', bh.toggleLeft)
            ctr.set_button_down_trigger('R1', bh.toggleRight)
        except:
            pass
    ...
    if cfg.CONDITIONAL_CMDS:
        ccmds = ConditionalPart()
        V.add(ccmds, outputs=['user/conditional_command',
              'conditional_command/label'])
        try:
            ctr.set_button_down_trigger('L1', ccmds.left)
            ctr.set_button_down_trigger('R1', ccmds.right)
        except:
            pass

        inputs = ['cam/image_array', "user/conditional_command"]
    ...
    aiLauncher = AiLaunch(cfg.AI_LAUNCH_DURATION,
                          cfg.AI_LAUNCH_THROTTLE, cfg.AI_LAUNCH_KEEP_ENABLED)
    ...
    if cfg.RECORD_DURING_AI:
        V.add(AiRecordingCondition(), inputs=[
    ...
    if cfg.CONDITIONAL_CMDS:
        inputs += ['user/conditional_command']
        types += ['int']
    ...
    if cfg.HAVE_MQTT_TELEMETRY:
        ...
        V.add(tel, inputs=telem_inputs, outputs=[
              "tub/queue_size"], threaded=True)
